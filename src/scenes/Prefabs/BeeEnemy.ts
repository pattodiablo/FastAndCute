...
export default class BeeEnemy extends SpineGameObject {
	...
	constructor(scene: Phaser.Scene, plugin: SpinePlugin, x: number, y: number, dataKey?: string, atlasKey?: string, skin?: string, boundsProvider?: SpineGameObjectBoundsProvider, xargs?: any) {
		super(scene, plugin, x ?? 0, y ?? 0, dataKey ?? "BeeEnemy", atlasKey ?? "BeeEnemy-atlas", boundsProvider ?? new SkinsAndAnimationBoundsProvider("Idle", ["default"]));
